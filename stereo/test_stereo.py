...
for i in range(1,25):   # Put the amount of pictures you have taken for the calibration inbetween range(0,?) wenn starting from the image number 0
    t= str(i)
    print(t)
    if i < 10:
        ChessImaR= cv2.imread('images/right_0'+t+'.ppm',0)    # Right side
        ChessImaL= cv2.imread('images/left_0'+t+'.ppm',0)    # Left side
    else:
        ChessImaR= cv2.imread('images/right_'+t+'.ppm',0)    # Right side
        ChessImaL= cv2.imread('images/left_'+t+'.ppm',0) 
...
##        frameR[line*20,:]= (0,255,0)    
    ...
    grayR = np.dot(Right_nice.astype(np.float64) , coeffs).sum(axis=-1) 
    grayR = grayR.astype(Right_nice.dtype)

    grayL = np.dot(Left_nice.astype(np.float64) , coeffs).sum(axis=-1) 
    grayL = grayL.astype(Left_nice.dtype)
